...
* **Do not re-sanitise stored content.** An earlier note here proposed passing old `links.content` through `p3k\XRay\Formats\Format::sanitizeHTML()`, for rows the hosted XRay stored before v2.0.1. Measured on a copy of production, 12% of rows change and the change is mostly the removal of `rel="nofollow"` from links, because XRay's HTMLPurifier configuration allows no `rel` attribute. That would strip nofollow from roughly 70,000 rows, which is worse than leaving them. Only 50 stored rows come from github.com. The fix belongs upstream in XRay, which should keep `rel` on `<a>`; until then, leave stored content alone.
